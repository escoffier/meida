...
void PtzControl::Process()
{
	...
	long userId = SdkUserManager::GetInstance()->GetSdkUser(device_->userId)->GetUserId();
	long channelId = SdkUserManager::GetInstance()->GetSdkUser(device_->userId)->GetChannelId();
	if (icmd < 0x4A)
	{
		...
		if (!DecodePTZCommand(device_->userId, ptzcmd_, dwPTZCommand, dwStop, dwSpeed))
		{
			...
			{
				response_(device_->userId, -1);
			}
			LOG(WARNING)<<"解析云台命令出错,SIPID:" << device_->userId;
			return;
		}
		...
		if (!NET_DVR_PTZControlWithSpeed_Other(userId, channelId, dwPTZCommand, dwStop, dwSpeed))
		{
			...
			LOG(WARNING) <<  "云台控制出错,SIPID:" << device_->userId << "错误原因:" << NET_DVR_GetLastError();
			return;
		}
		LOG(INFO) <<  "设置ptz成功,SIPID:" << device_->userId << " cmd = " << icmd << "  HKcmd =" << dwPTZCommand << " stop=" << dwStop << " speed=" << dwSpeed;
	...
	else if ((0x80 < icmd) && (icmd < 0x84))
	{
		...
		if (!DecodePTZPreset(ptzcmd_, dwPTZPresetCmd, dwPresetIndex))
		{
			...
			LOG(WARNING) << "解析云台预置点命令出错,SIPID:" << device_->userId;
		...
		if (!NET_DVR_PTZPreset_Other(userId, channelId, dwPTZPresetCmd, dwPresetIndex))
		{
			...
			LOG(WARNING) << "设置云台预置点命令出错,SIPID:" << device_->userId << "错误原因:" << NET_DVR_GetLastError();
			return ;
		}
		LOG(INFO) << "设置预置点成功,SIPID:" << device_->userId << " cmd = " << icmd << "  HKcmd =" << dwPTZPresetCmd << " INDEX=" << dwPresetIndex;
	}
	else if ((0x83 < icmd) && (icmd < 0x89))
	{
		...
		if (!NET_DVR_PTZCruise_Other(userId, channelId, dwPTZCruiseCmd, byCruiseRoute, byCruisePoint, wInput))
		{
			...
			LOG(WARNING) << "设置云台Cruise命令出错,SIPID:" << device_->userId << "错误原因:" << NET_DVR_GetLastError();

			return ;
		}
		LOG(INFO) << "设置Cruise成功,SIPID:" << device_->userId << " cmd = " << icmd << "  HKcmd =" << dwPTZCruiseCmd << " rute=" << byCruiseRoute << " point=" << byCruisePoint << "  input=" << wInput;
	}
	else if ((0x88 < icmd) && (icmd < 0x8B))
	{
		LOG(WARNING) << "设置云台扫描海康不支持,SIPID:" << device_->userId << " cmd = " << icmd;
	}
	else
	{
		LOG(WARNING) << "设置云台命令超国标范围,SIPID:" << device_->userId << " cmd = " << icmd;
	}
