...
{
	std::shared_ptr<SdkUser> sdkUser = SdkUserManager::GetInstance()->GetSdkUser(device_->userId);
	if (sdkUser == nullptr)
	{
		LOG(ERROR) << "不存在的设备"<< device_->userId;
		return;
	}
	if ( ! (sdkUser->IsOnline()) )
	{
		LOG(ERROR) << "设备: " << device_->userId<<"没有登录";
	...
	if (sdkUser->IsPlaying())
	{
		...
		response_(device_->userId, 0);
		LOG(ERROR)<< "实时预览已开启,SIPID:" << device_->userId;
		return ;
	}
	...
	LONG lPlayHandle = 0;
	sdkUser->SetDeviceId(device_->userId);
	sdkUser->SetDataProc(dataProc_);
	//DataProc *dataProc = new DataProc(dataProc_, device_->userId);
	//std::shared_ptr<DataProc> dataProc = std::make_shared<DataProc>(dataProc_);
